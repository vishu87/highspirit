...
?>
	...
	<table cellspacing="0" cellpaddin="0" width="100%">
		<?php 
			...
			if($flag_new == 1){
				$array_day_sum = array();
				$sql_fetch = mysql_query("SELECT * from sale_english_sum_final where shop_id='$shop_id' and timestamp_day='$time_now'");
				...
				$sql_fetch = mysql_query("SELECT * from sale_english where shop_id='$shop_id' and timestamp_day='$time_now' ");
				while($row_fetch = mysql_fetch_array($sql_fetch)){
				...
				$sql_fetch = mysql_query("SELECT * from sale_english_final where shop_id='$shop_id' and timestamp_day='$time_now' ");
				while($row_fetch = mysql_fetch_array($sql_fetch)){
				...
				$sql_fetch = mysql_query("SELECT * from sale_english where shop_id='$shop_id' and timestamp_day='$time_ytr' ");
				while($row_fetch = mysql_fetch_array($sql_fetch)){
					$array_day[$row_fetch["type_id"]][$row_fetch["size_id"]]["old_stock"] = $row_fetch["old_stock"];
					$array_day[$row_fetch["type_id"]][$row_fetch["size_id"]]["stock_in"] = $row_fetch["stock_in"];
					$array_day[$row_fetch["type_id"]][$row_fetch["size_id"]]["stock_transfer"] = $row_fetch["stock_transfer"];
				...
				$array_day_final = array();
				$sql_fetch = mysql_query("SELECT * from sale_english_final where shop_id='$shop_id' and timestamp_day='$time_ytr' ");
				while($row_fetch = mysql_fetch_array($sql_fetch)){
					$array_day_final[$row_fetch["size_id"]]["old_stock"] = $row_fetch["old_stock"];
					$array_day_final[$row_fetch["size_id"]]["stock_in"] = $row_fetch["stock_in"];
					$array_day_final[$row_fetch["size_id"]]["stock_transfer"] = $row_fetch["stock_transfer"];
								...
								for($i = 1; $i <= $total_sizes; $i++) { 
									if($flag_new == 1){
										...
										if(!$array_day[$row_type["id"]][$i]["rate"] || $array_day[$row_type["id"]][$i]["rate"] == 0 || $array_day[$row_type["id"]][$i]["rate"] == ''){
											$sql_fetch_rate = mysql_query("SELECT rate from sale_english where shop_id='$shop_id' and type_id = $row_type[id] and size_id = $i order by timestamp_day desc limit 1 ");
