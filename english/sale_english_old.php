...
<?php
	...
	$res_shop = mysql_query("SELECT id, name from shops_english where district='$current_district' and group_name = '$current_group' ");
...
<div style="font-size:14px;">
	...
	<div style="padding:7px 10px; background:#ccc; display:none;" id="shop_list">
		<?php  
			mysql_query('SET character_set_results=utf8');
			$sql_shop = mysql_query("SELECT name from shops_english where district='$current_district' and group_name = '$current_group' ");
			$count =0;
...
$check_old = mysql_query("SELECT timestamp_day from sale_english where shop_id='$row_shop[id]' order by timestamp_day asc limit 1");
...
$check_skip = mysql_query("SELECT timestamp_day from sale_english where shop_id='$row_shop[id]' order by timestamp_day desc limit 1");
...
$sql_new = mysql_query("SELECT id from sale_english where shop_id='$row_shop[id]' and timestamp_day = $time_now ");
if(mysql_num_rows($sql_new) > 0){
	...
	if($priv == 0) {
		...
		$sql_ck_last = mysql_query("SELECT timestamp_day from sale_english where shop_id='$row_shop[id]' order by timestamp_day desc");
		$row_ck_last = mysql_fetch_array($sql_ck_last);
		if($row_ck_last["timestamp_day"] == $time_now)echo '<div align="center" style="margin:20px;" ><a href="process_english_delete.php?current_shop='.$current_shop.'&amp;type=3&amp;shop_id='.$row_shop["id"].'&amp;timestamp='.$time_now.'&amp;dis='.$current_district.'&amp;grp='.$current_group.'" style="padding:10px; background:#000; color:#fff;">यह डॉटा डिलीट कर दे</a></div>';
	}
}
...
?>
	...
	<table cellspacing="0" cellpaddin="0" width="100%">
		<?php 
			...
			mysql_query('SET character_set_results=utf8');
			$query_range = mysql_query("SELECT * from english_range order by id  limit 1");
			while($row_range = mysql_fetch_array($query_range)){
				echo '<tr>';
						echo '<td></td>';
						$count_det =0;
						foreach ($ar_details as $detail) {
							echo '<td ';
							if($count_det != 7) echo 'colspan="4"';
							echo '>'.$detail.'</td>';
							$count_det++;
						}	
				echo '</tr>';
				...
				while($row_type = mysql_fetch_array($query_type)){
					...
					foreach ($in_details as $detail) {
						switch ($detail) {
							case 'total_sum':
								if($flag_new == 1){
										$sql_fetch = mysql_query("SELECT ".$detail." from sale_english_sum_final where shop_id='$shop_id' and type_id='$row_type[id]' and timestamp_day='$time_now' limit 1 ");
										$row_fetch = mysql_fetch_array($sql_fetch);
										$value = $row_fetch[$detail];
								} else {
									$value='';
								}
							...
							case 'total_cost':
								for($i = 1; $i <= $total_sizes; $i++) {
									if($flag_new == 1){
										$sql_fetch = mysql_query("SELECT ".$detail." from sale_english where shop_id='$shop_id' and type_id='$row_type[id]' and size_id='$i' and timestamp_day='$time_now' limit 1 ");
										$row_fetch = mysql_fetch_array($sql_fetch);
										$value = $row_fetch[$detail];
									}else {
										$value='';
							...
							case 'final_stock':
								for($i = 1; $i <= $total_sizes; $i++) { 
									if($flag_new == 1){
										...
										$sql_fetch = mysql_query("SELECT final_stock from sale_english where shop_id='$shop_id' and type_id='$row_type[id]' and size_id='$i' and timestamp_day='$time_ytr' limit 1 ");
										$row_fetch = mysql_fetch_array($sql_fetch);
										$value = $row_fetch["final_stock"];
									}
							...
							case 'old_stock':
								for($i = 1; $i <= $total_sizes; $i++) { 
									$query_ytr =  mysql_query("SELECT final_stock from sale_english where shop_id='$shop_id' and type_id='$row_type[id]' and timestamp_day='$time_ytr' limit 1 ");
									$flag_old = (mysql_num_rows($query_ytr)>0)?0:1;
							...
							case 'rate':
								for($i = 1; $i <= $total_sizes; $i++) { 
									if($flag_new == 1){
										...
										$sql_fetch = mysql_query("SELECT rate from sale_english where shop_id='$shop_id' and type_id='$row_type[id]' and size_id='$i' order by timestamp_day desc limit 1 ");
										$row_fetch = mysql_fetch_array($sql_fetch);
										$value = $row_fetch["rate"];
									}
			...
			foreach ($in_details as $detail) {
				if($detail == 'old_stock' || $detail == 'initial_stock' || $detail == 'final_stock' ){
					for($i=1;$i<=$total_sizes;$i++) {
						if($flag_new == 1){
							$sql_fetch = mysql_query("SELECT ".$detail." from sale_english_final where shop_id='$shop_id' and size_id='$i' and timestamp_day='$time_now' limit 1 ");
							$row_fetch = mysql_fetch_array($sql_fetch);
							$value = $row_fetch[$detail];
						}  else {
							$sql_fetch = mysql_query("SELECT final_stock from sale_english_final where shop_id='$shop_id' and size_id='$i' and timestamp_day='$time_ytr' limit 1 ");
							$row_fetch = mysql_fetch_array($sql_fetch);
							$value = $row_fetch["final_stock"];
						}
					...
					if($flag_new == 1){
						$sql_fetch = mysql_query("SELECT ".$detail." from sale_english_sum_final where shop_id='$shop_id' and type_id='0' and timestamp_day='$time_now' limit 1 ");
						$row_fetch = mysql_fetch_array($sql_fetch);
						$value = $row_fetch[$detail];
					} else {$value=''; }
	...
	<div align="center" style="margin:20px 0" id="final_fields">
		<?php
			$sql_fetch = mysql_query("SELECT * from sale_english_sum_final where shop_id='$shop_id' and type_id='0' and timestamp_day='$time_now' limit 1 ");
			$row_fetch = mysql_fetch_array($sql_fetch);
		?>
		...
		<tr>
			<td><input type="text"  autocomplete="off" name="other_money" id="other_money" value="<?php echo $row_fetch["other_money"]?>" /></td><td>अन्य जमा</td>
			<td><input type="text"  autocomplete="off" name="remark" value="<?php echo $row_fetch["remark"]?>" /></td><td>रिमार्क</td>
		</tr>
		<tr>
			<td colspan="3"><input type="text" readonly id="final_total" autocomplete="off" name="final_total" value="<?php echo $row_fetch["final_total"]?>" /></td><td>कुल योग</td>
		</tr>
		...
		<?php
			if($flag_new == 0){
				?>
					<button type="button" onclick="submit_english_form()" style="padding:5px 10px">जमा करे</button>
				<?php
			}
		?>
	</div>
	...
	var brand_type;
 	...
 	function submit_english_form(){
 		...
 		if( ($("#total_old_stock_1").val() == '' || $("#total_old_stock_1").val() == 0) && ($("#total_old_stock_2").val() == '' || $("#total_old_stock_2").val() == 0) && ($("#total_old_stock_3").val() == '' || $("#total_old_stock_3").val() == 0) && ($("#total_old_stock_4").val() == '' || $("#total_old_stock_4").val() == 0) ){
 				alert('कृपया फार्म भरे ');
 			}
 			else {
 					 		$(".english_form").each( function(){ 
						var val = $(this).serialize();
						$.post("process_english.php?shop_id="+$("#shop_id").val()+"&current_id="+$("#current_id").val()+"&date_en="+$("#date_en").val()+"&month_en="+$("#month_en").val()+"&year_en="+$("#year_en").val() ,val, function(data){
						});
						count++;

						if(count == total){
							var val = $("#final_form").serialize();
							$.post("process_english_final.php?shop_id="+$("#shop_id").val()+"&current_id="+$("#current_id").val()+"&date_en="+$("#date_en").val()+"&month_en="+$("#month_en").val()+"&year_en="+$("#year_en").val() ,val, function(data){
								if(data == 'success'){
