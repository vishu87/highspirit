...
<?php
	...
	while ($row = mysql_fetch_array($query)) {
			...
			$query_sale = mysql_query("SELECT id from sale_english where shop_id='$row[id]' order by timestamp_day asc ",$link_local);
			while ($row_sale = mysql_fetch_array($query_sale)) {
				$query = mysql_query("SELECT id from sale_english where id = $row_sale[id] limit 1",$link);
			...
			//FINAL SALE
			$query_sale = mysql_query("SELECT * from sale_english_final where shop_id='$row[id]' order by timestamp_day asc",$link_local);
			while ($row_sale = mysql_fetch_array($query_sale)) {
				$query = mysql_query("SELECT id from sale_english_final where id = $row_sale[id] limit 1",$link);
			...
			//FINAL SALE SUM
			$query_sale = mysql_query("SELECT * from sale_english_sum_final where shop_id='$row[id]' order by timestamp_day asc",$link_local);
			while ($row_sale = mysql_fetch_array($query_sale)) {
				$query = mysql_query("SELECT id from sale_english_sum_final where id = $row_sale[id] limit 1",$link);
