...
if($shop == 0){
	$shops = array();
	$res_shop = mysql_query("SELECT id from shops_beer where group_name = '$grp'");
	...
	$res_shop = mysql_query("SELECT id, name, group_name from shops_beer where id = '$shop' limit 1 ");
...
$query = "SELECT type_id, size_id, SUM(old_stock) as old_stock from sale_beer where shop_id IN (".$shop_string.") and timestamp_day = $time_pre GROUP BY type_id, size_id";
...
$query = "SELECT type_id, size_id, SUM(final_stock) as final_stock from sale_beer where shop_id IN (".$shop_string.") and timestamp_day = $time_nxt GROUP BY type_id, size_id";
$result = mysql_query($query);
while ($row = mysql_fetch_array($result)) {
...
$array_sale = array();
$query = "SELECT type_id, size_id, SUM(stock_in) as stock_in, SUM(stock_transfer) as stock_transfer, SUM(sale) as sale, SUM(rate*sale) as rate, SUM(total_cost) as total_cost from sale_beer where shop_id IN (".$shop_string.") and timestamp_day between $time_pre and $time_nxt GROUP BY type_id, size_id";
...
$query = "SELECT type_id, SUM(total_sum) as total_sum from sale_beer_sum_final where shop_id IN (".$shop_string.") and timestamp_day between $time_pre and $time_nxt GROUP BY type_id";
...
// 			$query = "SELECT total_sum, timestamp_day from sale_beer_sum_final where shop_id IN (".$shop_string.") and timestamp_day between $time_pre and $time_nxt and type_id = 0 order by timestamp_day asc ";
