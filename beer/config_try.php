...
<?php
	...
	if(isset($_GET["shop"])){
		...
		$query = mysql_query("SELECT id,name from shops_beer", $link_local);
	}
	while ($row = mysql_fetch_array($query)) {
		
		$q_ts = mysql_query("SELECT timestamp_day from sale_beer where shop_id='$row[id]' order by timestamp_day desc limit 1 ",$link_local);
		if(mysql_num_rows($q_ts) > 0){
			$row_ts = mysql_fetch_array($q_ts);
			$last_timestamp = $row_ts["timestamp_day"] - 90*86400;

			$query_sale = mysql_query("SELECT * from sale_beer where shop_id='$row[id]' and timestamp_day <= $last_timestamp ",$link_local);
			while ($row_sale = mysql_fetch_array($query_sale)) {
				$query1 = "INSERT INTO sale_beer (";
				$query2 = ")VALUES (";
				$query3 = ")";
			...
			//FINAL SALE
			$query_sale = mysql_query("SELECT * from sale_beer_final where shop_id='$row[id]' and timestamp_day <= $last_timestamp ",$link_local);
			while ($row_sale = mysql_fetch_array($query_sale)) {
				$query1 = "INSERT INTO sale_beer_final (";
				$query2 = ")VALUES (";
				$query3 = ")";
			...
			//FINAL SALE SUM
			$query_sale = mysql_query("SELECT * from sale_beer_sum_final where shop_id='$row[id]' and timestamp_day <= $last_timestamp ",$link_local);
			while ($row_sale = mysql_fetch_array($query_sale)) {
				$query1 = "INSERT INTO sale_beer_sum_final (";
				$query2 = ")VALUES (";
				$query3 = ")";
